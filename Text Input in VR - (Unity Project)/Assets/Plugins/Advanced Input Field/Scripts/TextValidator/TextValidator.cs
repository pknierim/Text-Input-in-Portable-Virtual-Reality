		...
		internal char ValidateChar(char ch, char[] text, int textLength, int pos, int caretPosition, int selectionStartPosition)
		{
			...
			if(Validation == CharacterValidation.Integer || Validation == CharacterValidation.Decimal)
			{
				...
				if(!cursorBeforeDash || dashInSelection)
				{
					...
					if(ch == '.' && Validation == CharacterValidation.Decimal && !Util.Contains('.', text, textLength)) return ch;
					if(ch == ',' && Validation == CharacterValidation.Decimal && !Util.Contains('.', text, textLength)) return '.';
				}
			}
